...
import cd.go.authentication.ldap.executor.VerifyConnectionRequestExecutor;
...
public class VerifyConnectionRequestExecutorTest {
    ...
    public void shouldReturnNonEmptyValidationResultIfProfileIsInvalid() throws Exception {
        ...
        assertThat(result, contains(new ValidationError("LDAP: error code 49 - INVALID_CREDENTIALS: Bind failed: ERR_229 Cannot authenticate user uid=no-such-user,ou=system")));
    }

    @Test
    public void shouldReturnNonEmptyValidationResultFailedToParseLdapURL() throws Exception {
        Map<String, String> profileMap = new HashMap<>();
        profileMap.put("Url", "some-invalid-url");
        profileMap.put("SearchBases", "ou=users,ou=system");
        profileMap.put("ManagerDN", "uid=no-such-user,ou=system");
        profileMap.put("Password", "secret");
        profileMap.put("SearchFilter", "uid");
        ...
        assertThat(result, contains(new ValidationError("Cannot parse url: some-invalid-url")));
    }
}